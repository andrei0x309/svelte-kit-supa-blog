<script lang="ts">
    ...
    export let data: IPost["signature"] | string
    let expanded = false
    let json : IPost["signature"]
    let isValid = true
    onMount(() => {
        try {
...
aria-hidden
><svg  class="w-4 h-4 inline mr-2" fill="currentColor" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg"><title/><path d="M76.79,19.16,22.2,49.27,0,119.55,36.56,83a5.93,5.93,0,0,1,1.06-.83,13.68,13.68,0,1,1,8.22,8.22A6,6,0,0,1,45,91.44L8.45,128l70.28-22.2,30.12-54.59L128,32.05,95.95,0Z"/></svg>
EVM Signature
